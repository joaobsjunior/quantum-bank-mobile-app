...
expected_origin="GATEWAY_BASE_URL=http://localhost:8080"
...
if ! grep -Fxq "$expected_origin" "$config_file"; then
  echo "mobile gateway config must contain exactly: $expected_origin" >&2
  exit 1
fi

config_lines="$(grep -Ev '^[[:space:]]*(#|$)' "$config_file" | wc -l | tr -d '[:space:]')"
if [[ "$config_lines" != "1" ]]; then
  echo "mobile gateway config must expose only GATEWAY_BASE_URL" >&2
  exit 1
fi
...
for pattern in BACKEND_BASE_URL "backend:" "localhost:8081" "http://backend"; do
